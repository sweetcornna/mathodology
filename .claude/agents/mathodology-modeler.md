...
End your work with an S2 `handoff:` yaml block (schema in the mathodology-award-gates skill; lint with `lint_run.py handoff`). Beyond the standard keys it carries the extra key `innovation_ledger: [{id: INN-1, claim: ..., evidence: ..., defense: ...}]`. The block must convey:
