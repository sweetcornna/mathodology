...
The root `.mcp.json` registers the keyless `search` MCP server (free-search-mcp) that `mathodology-evidence-search` uses, so a clone has working evidence search with no per-user setup.
...
- `mathodology-evidence-search` for external evidence work: the `search` MCP tool stack (free-search-mcp), category routing to the literature and dataset databases, the citation verification protocol, the reproducibility boundary, and the WebSearch/WebFetch fallback.
